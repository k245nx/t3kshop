			...
			<div id="centralblock">
				...
				<aside id="centrale">
					...
					<section id="adaptivesection">
						<h3>Carrello</h3>
						<% if(cartToDisplay.size()>0){ %>
							<form action="updateCart" method="post">
								<table id=cartTable>
									<th>Elim.</th>
									<th id=cartitemth>Articolo</th>
									<th>Q.tà</th>
									<th id=cartpriceth>Totale</th> 
									<% for(int i=0; i<cartToDisplay.size(); i++){ %>
										 <tr>
										 	<td><input type="checkbox" name="<%="deleteItem"+i%>" value="<%=cartToDisplay.get(i).getId()%>"></td>
										 	<td id=cartitemtd><a href="<%="product?id=" + cartToDisplay.get(i).getId() %>"><%=cartToDisplay.get(i).getNome()%></a></td>
										 	<td><input type="text" size="1" name="<%="quantita"+i%>" value="<%=currentCart.get(i).getQuantita()%>"></td>
										 	<td id=cartpricetd><%="€" + cartToDisplay.get(i).getPrezzo()%></td>
										 </tr>
									<% } %>
								</table>
								<% 	double fixedFloatPrice, totale = 0;
									for(int i=0; i<cartToDisplay.size(); i++){
										fixedFloatPrice = Double.parseDouble(cartToDisplay.get(i).getPrezzo().replace(",", "."));
										totale += fixedFloatPrice*currentCart.get(i).getQuantita();
									}
									DecimalFormat df = new DecimalFormat("#.00");
								%>
								<div style="text-align: right; margin-right: 5px; "><p style="width: 100%;"> Totale Ordine € <%=df.format(totale)%></p></div>
								<input style="float: left;" type="submit" name="update" value="Aggiorna">
								<input style="float: right; margin-right: 20px;" type="submit" name="checkout" value="Checkout">
							</form>
						<% }else{ %>
							<p>Al momento il tuo carrello è vuoto.</p>
						<% } %>
					</section>
