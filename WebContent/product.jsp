				...
				<aside id="centrale">
					...
					<section id=adaptivesection>
						...
						<div id=mainimg>
							...
							<% if(prodotti.getProdotti().get(0).getQuantita()<=0){ %>
								<div style="vertical-align: middle; border-top: 1px solid #e3e3e3; border-bottom: 1px solid #e3e3e3; padding: 5px;"> Non disponibile <img src="images/non_disponibile.png"></div>
							<% }else{ %>
								...
								<form style="border-top: 1px solid #e3e3e3; border-bottom: 1px solid #e3e3e3; padding: 5px;" action="addWishlist" method="post">
									Aggiungi alla wishlist <input type="hidden" name="id" value="<%=(prodotti.getProdotti()).get(0).getId()%>">
						...
						<div style="text-align: center; float:left; border-top: 1px solid #e3e3e3; padding: 10px;">
						<% for(int i=0; i<5 && i<(prodotti.getProdotti()).size();i++){ %>
							<div id=suggestitem>
