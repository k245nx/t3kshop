...
<%
	...
	manageOrder.setUserOrder(username);
					...
					<section id="adaptivesection">
						...
						<% if(ordini.size()>0){ %>
							<% for(int i=0;i<ordini.size();i++){ %>
								<p>Nr. ordine: <%=ordini.get(i).getId()%></p>
								<div id=myordersitem>
									<% double fixedFloatPrice=0, sum=0;
										for(int j=0; j<ordini.get(i).getArticoli().size();j++){
											...
											<a href="<%="product?id=" + (prodotti.getProdotti()).get(0).getId() %>"><img id=wishlistimg width="100" height="100" src="<%="images/products/" + (prodotti.getProdotti()).get(0).getImmagine()%>" alt="product"></a>
											<div id=wishlistdescitem>
												<a href="<%="product?id=" + (prodotti.getProdotti()).get(0).getId() %>"><%=(prodotti.getProdotti()).get(0).getNome()%></a>
												<h5 style="padding-top: 10px;"><%=(prodotti.getProdotti()).get(0).getBreveDescrizione() %></h5>
