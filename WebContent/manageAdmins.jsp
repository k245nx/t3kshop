...
<html lang="it">
	<head>
		...
		<script src="manageAdminScript.js"></script>
				...
				<aside id="centrale">
					...
					<section id="adaptivesection">
						<%	ArrayList<Admin> adminArray=new ArrayList<Admin>();
							adminArray=manageAdmin.getAdmins();
						%>
						
						<h3>Aggiungi un nuovo amministratore</h3>	
						<form name="InsertAdmin" onsubmit="return ManageAdmins()" action="addAdmin" method="post">
							<div id=registerattributes>
								...
								<br>
								<input type="submit" name="Invia" style="clear:both;" value="Inserisci Admin">
							</div>
						...
						<h3>Rimuovi un altro amministratore</h3>
						<%if(adminArray.size()==1){%>
							<p>Al momento sei l'unico amministratore</p>
						<%}else{%>
							<form style="margin-left: 50px;" action="DeleteAdmin" name="DeleteAdminForm" method="post"  onsubmit="return ManageAdmins()">
									<select name="admin">
										<%for(int i=0;i<adminArray.size();i++){
											if(!adminArray.get(i).getUsername().equalsIgnoreCase(username)){%>
												<option name="adminOptions" value="<%=adminArray.get(i).getUsername()%>"><%=adminArray.get(i).getUsername()%></option>
											<%}
										}%>
									</select>
									<input type="submit" name="cancella" value="Rimuovi">
							</form>
						<%}%>
						<br>
